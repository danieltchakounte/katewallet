...
from configuration.response import STAKE_MESSAGE, UNEXISTANT, STAKE_STATUS, UNSTAKE_VALID_COMMAND
            ...
            try:
                ...
                try:
                    ...
                    if validator.startswith('injvaloper'):
                        ...
                        if (exist_wallet):
                            stake = await database.unstake(update.effective_chat.id, amount, validator)
                    ...
                    text = '*Validator address missing*\nTry with `/unstake amount validator_address`'
                
            except:
                text = '*Amount to unstake is not a valid number*. Please review it and try again'
    except :
        text=  UNSTAKE_VALID_COMMAND['en']
